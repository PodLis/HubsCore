...
public class StringUtils {
    ...
    public static String setPlaceholders(String s, String... data) {
        if (data.length <= 1) {
            ...
            return setPlaceholders(s, Arrays.copyOf(data, data.length - 2)).replaceAll("%" + data[data.length-2] + "%", Matcher.quoteReplacement(data[data.length-1]));
        }
    }

    public static List<String> setPlaceholders(List<String> strings, String... data) {
        strings.forEach(s -> s = setPlaceholders(s, data));
        return strings;
    }
